<?php
	...
	if ($forceAttackingText) {
		$rq = selectRandomInArray($GLOBALS["PROMPTS"]["afterattack"]["cue"]);
		if (strlen($rq) < 1) error_log(" funcret.php: selectRandomInArray empty list PROMPTS[afterattack][cue] "); // debug		
		$returnFunctionArray[] = array('role' => $LAST_ROLE, 'content' => $rq." {$GLOBALS["HERIKA_NAME"]}: ");
	} else
