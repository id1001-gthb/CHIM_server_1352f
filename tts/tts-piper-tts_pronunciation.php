...
/*
Alleviate Piper-TTS English mispronunciation.
tts-piper-tts_pronunciation.php
*/

$GLOBALS['PIPERTTS_PRONUNCIATION_ADJUST'] = true; // set false to hear original pronunciation 	
$GLOBALS['PIPERTTS_PRONUNCIATION_DEBUG'] = false; // for tests in tts-test.php, true to display changed text 	
...
function pipertts_adjust_pronunciation($s_text="", $language="EN") {
	...
	if (strlen($s_text) > 0) {
		...
		if ($lang == "EN") {
			...
			$notgood = array( 
			//"don't", "can't",  "couldn't",  "didn't",  "doesn't",  "hadn't",  "hasn't",  "haven't",  "I've",   "aren't", 
			...
			$ok      = array(
			//"donth",   "kanth",   "could not", "did not", "dosnth", "had not", "has not", "have not", "I have", "aanth", // variants: do not, can not
			...
			//"", "", "", 
			"humf", "humhmh", "muhmh", "buhr", "guhr", "shish", "zuzz" 
			); 

			$encoding = mb_detect_encoding($s_text); // some LLMs could send UTF-8 punctuation
			...
			//error_log("pipertts adjusted " . $encoding . ": " . $s_clean);
		}  // --- endif EN

	} else
		Logger::warn("pipertts adjust contractions empty text! ");

	return $s_clean; 
}

function pipertts_pronunciation_adjust_enabled() {
	return isset($GLOBALS['PIPERTTS_PRONUNCIATION_ADJUST']) ? $GLOBALS['PIPERTTS_PRONUNCIATION_ADJUST'] : true; //disable only when explicit false
}

function pipertts_pronunciation_debug_enabled() {
	return isset($GLOBALS['PIPERTTS_PRONUNCIATION_DEBUG']) ? $GLOBALS['PIPERTTS_PRONUNCIATION_DEBUG'] : false;
}
